...
import { Expr, query as q, Role } from "faunadb"
...
export class UserDocument extends Document<User> {
  ...
  public static async fromToken(token: string): Promise<UserDocument> {
    const client = authenticatedClient(token)
    const res = await client.query<QueryResponse<User>>(q.Get(q.CurrentIdentity()))
   ...
   * during signup.
   */
  public static async create(input: CreateUser, token: string): Promise<UserDocument> {
    const client = authenticatedClient(token)
    console.log({input})
    const validatedInput = await CreateUserValidation.parseAsync(input).catch((err) => {
      throw new Error(`Unable to parse CreateUserInput: ${err}`)
    })

    /**
     * Transform the CreateUser input to UserSchema.
     */
    const data = await UserSchemaValidation.parseAsync({
      name: validatedInput.name,
      email: validatedInput.email,
      role: validatedInput.role,
    }).catch(err=>{
      throw new Error(`UserSchemaValidation: ${err}`)
    })
    ...
    const res = await client.query<{
      ...
      q.Let(
        {
          ...
          token: q.Do(
            q.Create(q.Collection(COLLECTION_USERS), {
              credentials: { password: validatedInput.password },
              data,
            }),
            q.Login(q.Match(q.Index(INDEX_USER_BY_EMAIL), q.Casefold(validatedInput.email)), {
      ...
      throw new Error(`Unable to create document in collection: ${err}`)
    })
     ...
     */
    const userData = await UserValidation.parseAsync({
      ...res.user.data,
      token: res.secret,
    }).catch(err=>{
      throw new Error(`User response from fauna is invalid: ${err}`)
    })

    return new UserDocument(userScopedClient, res.user.ref, res.user.ts, userData)
  }
      ...
      q.Let(
        {
           ...
           * }.
           */
          token: q.Login(q.Match(q.Index(INDEX_USER_BY_EMAIL), q.Casefold(validatedInput.email)), {
            password: validatedInput.password,
          }),
        },
  ...
  public static async fromEmail(email: string, token: string): Promise<UserDocument> {
    const client = authenticatedClient(token)
