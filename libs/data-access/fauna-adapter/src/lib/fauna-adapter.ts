    ...
    async getAdapter({ session, secret, ...appOptions }) {
      ...
      return {
        ...
        async deleteUser(userId) {
          await faunaClient.query(q.Delete(q.Ref(q.Collection(collections.User), userId)))
        },
        async linkAccount(
          userId,
          providerId,
        ...
        async deleteSession(sessionToken) {
          return await faunaClient.query(
            q.Delete(q.Select("ref", q.Get(q.Match(q.Index(indexes.Session), sessionToken)))),
          )
        },
          ...
          await faunaClient.query(
            q.Delete(
              q.Select(
                "ref",
                q.Get(
                  q.Match(q.Index(indexes.VerificationRequest), [hashToken(token), identifier]),
                ),
              ),
            ),
          )
