...
export class RefreshToken extends Document<z.infer<typeof RefreshToken.schema>> {
   ...
   */
  public static async deleteFromUser(client: Client, userId: string): Promise<void> {
    await client
      .query(
        q.Map(
  ...
  public static async create(
    ...
    try {
      ...
      const res = await client
        ...
        .catch((err) => {
          throw new Error(
            `Unable to create refresh_token: ${err}, received: ${JSON.stringify(data, null, "  ")}`,
          )
        })
