...
export class Price extends Document<z.infer<typeof Price.schema>> {
    ...
    try {
      const res = await client
        .query<QueryResponse<z.infer<typeof Price.schema>>>(
          q.Get(q.Match(q.Index(Price.index.bySymbolAndTime), [symbol, time.unix()])),
        )
        .catch(() => null)
   ...
   */
  public static async create(client: Client, input: z.infer<typeof Price.schema>): Promise<Price> {
    try {
      ...
      const res = await client.query<QueryResponse<z.infer<typeof Price.schema>>>(
        q.If(
          q.Exists(match),
          q.Get(match),
  ...
  public async delete(client: Client): Promise<void> {
    await client.query(q.Delete(q.Ref(q.Collection(Price.collection), this.id))).catch((err) => {
