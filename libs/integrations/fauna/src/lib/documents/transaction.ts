...
export class Transaction extends Document<z.infer<typeof Transaction.schema>> {
   ...
   */
  public static async fromId(client: Client, id: string): Promise<Transaction | null> {
    try {
      ...
      throw new Error(`Unable load assetDocument from asset: ${err}`)
    }
  }
  /**
   * Load all transactions from a user
   */
  public static async fromUserId(client: Client, userId: string): Promise<Transaction[]> {
    try {
  ...
  public static async create(
    ...
    try {
      ...
      const res = await client.query<QueryResponse<z.infer<typeof Transaction.schema>>>(
